...
def compute_policy_loss(old_log_prob, ref_log_prob, log_prob, advantages, upgo_advantages, eos_mask, cliprange,
                        ...
                        kl_penalty_type, overlong_mask, reduce_loss_by_token):
    ...
    if not reduce_loss_by_token:
        pg_loss = torch.sum(pg_losses * eos_mask, dim=1) / seq_len_per_sample  # batch_size
    else:
        pg_loss = pg_losses  # batch_size x response_length
    ...
    if overlong_mask is not None:
        if len(pg_loss.shape) == 1:
            ...
            pg_loss_mask = eos_mask * overlong_mask.unsqueeze(-1)
