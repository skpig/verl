# recompile from https://github.com/OrangeX4/latex2sympy
...
def latex2sympy(sympy: str, variable_values={}):
    ...
    # Translate Matrix
    sympy = sympy.replace(r'\left[\begin{matrix}', r'\begin{bmatrix}', -1).replace(r'\end{matrix}\right]',
                                                                                   r'\end{bmatrix}', -1)
...
def convert_frac(frac):
    ...
    if (frac.lower.start == frac.lower.stop and frac.lower.start.type == PSLexer.DIFFERENTIAL):
        ...
        diff_op = True
    elif (lower_itv_len == 2 and frac.lower.start.type == PSLexer.SYMBOL and frac.lower.start.text == '\\partial' and
    ...
    if diff_op or partial_op:
        ...
        if (diff_op and frac.upper.start == frac.upper.stop and frac.upper.start.type == PSLexer.LETTER_NO_E and
                frac.upper.start.text == 'd'):
            return [wrt]
        elif (partial_op and frac.upper.start == frac.upper.stop and frac.upper.start.type == PSLexer.SYMBOL and
...
def latex(tex):
    ...
    result = result.replace(r'\left[\begin{matrix}', r'\begin{bmatrix}',
                            -1).replace(r'\end{matrix}\right]', r'\end{bmatrix}', -1)
