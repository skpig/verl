...
def compute_score(solution_str, ground_truth, **kwargs):
    ...
    assert 'starting_point' in ground_truth, ground_truth
    assert 'ending_point' in ground_truth, ground_truth

    pred = extract_maze_answer(solution_str)
    if not pred:
        # ExtractAnswerFailed
        return extract_answer_failed_reward()
    distances_compressed = ground_truth['distances']
    score = verifier_maze(np.load(io.BytesIO(base64.b64decode(distances_compressed)))['distances'],
                          start_point=ground_truth['starting_point'],
                          end_point=ground_truth['ending_point'],
                          path=pred)
