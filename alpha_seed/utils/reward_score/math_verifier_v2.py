...
from alpha_seed.utils.reward_score.verifier.parser import extract_answer
...
def compute_score(solution_str, ground_truth, **kwargs) -> float:
    try:
        ref = submit_verifier.remote(solution_str, ground_truth)
...
def submit_verifier(decoded_text, verifier_feature):
    tag = 'none'
    if not decoded_text:
        return -0.1, tag
