...
class CollieSupplyVerifier(BaseVerifier):

    def verify(self, response: str, verifier_feature_dict: dict) -> VerifyResult:
        collie_label = json.loads(verifier_feature_dict['answer'])
        try:
            if response == "":
                raise ExtractAnswerFailed
            decoded_constrain = CONSTRAIN_MANAGER.load_constrain(collie_label)
            try:
                ...
                print(f"Error: failed to check constrain {ex}")
