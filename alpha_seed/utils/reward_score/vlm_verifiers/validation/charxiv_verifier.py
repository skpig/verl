...
def call_gpt_rp5(client, prompt, gpt_version, category, temperature=0.1):
    tb = ""
    output_text = ""
    for _ in range(5):
        tb = ""
        ...
        try:
            ...
            if category == 'descriptive':
                score = float(score_pred['score_T1'])
                gpt_response = str(score_pred['extract_answer_T1'])
            else:
                score = float(score_pred['score'])
                if 'extracted_answer' in score_pred:
                    gpt_response = str(score_pred['extracted_answer'])
                else:
                    gpt_response = ""
            break
            ...
            continue
    if tb:
        print(f"Invalid score: {repr(output_text)}")
        print(f'[VLM VERIFIER ERROR (CharXiv)] {repr(tb)}')
    return score, gpt_response

def llm_as_metric_for_charxiv(question, answer, predict, client, extra_info):
    gpt_version = "gpt-4o-2024-11-20"
    category = json.loads(extra_info)["task_name"]

    if category == 'descriptive':
        qid = json.loads(extra_info)["qid"]
        # question based on the template id
        ques = DESCRIPTIVE_GRADING_QMAP[qid]
        prefix = DESCRIPTIVE_GRADING_PREFIX\
            .replace("<|NUM_TRIPLETS|>", str(1))\
            .replace("<|OVERARCHING_QUESTION|>", ques)\
            .replace("<|JSON_KEYS|>", str(["extract_answer_T1", "score_T1"]))
        rubric_icl = get_rubric(qid)

        prompt = prefix + rubric_icl + populate_grading_inputs(predict, answer)

        # real_prompt = text_prompt_to_chat_prompt(prompt, role='user')
        # response_format={"type": "json_object"}
        score, gpt_response = call_gpt_rp5(client=client, prompt=prompt, gpt_version=gpt_version, category=category, temperature=0.1)

        return score, gpt_response
    else:
        # !!! raw_question from extra
        raw_question = json.loads(extra_info)["raw_question"]
        inst_category = json.loads(extra_info)["inst_category"]
        prompt = REASONING_GRADING_PREFIX + deepcopy(\
            REASONING_GRADING_INST[inst_category])\
            .replace("<|question|>", raw_question)\
            .replace("<|ground_truth|>", answer)\
            .replace("<|response|>", predict)
        # real_prompt = text_prompt_to_chat_prompt(prompt, role='user')
        response_format={"type": "json_object"}
        score, gpt_response = call_gpt_rp5(client=client, prompt=prompt, gpt_version=gpt_version, category=category, temperature=0.1)
        return score, gpt_response
...
class CharaxivVerifier(BaseVerifier):
    ...
    def verify(self, response: str, verifier_feature_dict: dict) -> VerifyResult:
        ...
        extra_info = verifier_feature_dict['extra_info']
        ...
        score, gpt_response = llm_as_metric_for_charxiv(query, answer, response, client, extra_info)
