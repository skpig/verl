...
nltk.data.path.append('/opt/tiger/nltk_data')
...
def count_sentences(text):
    ...
    if '/opt/tiger/nltk_data' not in nltk.data.path:
        nltk.data.path.append('/opt/tiger/nltk_data')
...
class PostscriptChecker(Instruction):
    ...
    def build_description(self, *, postscript_marker=None):
        ...
        self._postscript_marker = (postscript_marker.strip()
                                   if isinstance(postscript_marker, str) else postscript_marker)
...
class JsonFormat(Instruction):
    ...
    def check_following(self, value):
        value = (value.strip().removeprefix("```json").removeprefix("```Json").removeprefix("```JSON").removeprefix(
            "```").removesuffix("```").strip())
        try:
            json.loads(value)
        except ValueError:
...
def process_results_ifeval(doc, results):
    inp = InputExample(
        key=doc["key"],
...
IFEVAL_INSTRUCTION_CONFLICTS = {
    ...
    "detectable_content:number_placeholders": {"detectable_content:number_placeholders"},
    ...
    "detectable_format:number_highlighted_sections": {"detectable_format:number_highlighted_sections"},
    "detectable_format:multiple_sections": {
        "detectable_format:multiple_sections",
        "detectable_format:number_highlighted_sections",
        "language:response_language",
