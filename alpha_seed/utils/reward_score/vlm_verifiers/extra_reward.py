...
def filter_thinking_part_v2(response, eos_token=None):
    ...
    think_start = response.find(sot_token, response_start)
    think_end = response.rfind(eot_token, response_start)
    think_start_num = response.count(sot_token)
    think_end_num = response.count(eot_token)
    if think_start != -1 and think_end != -1 and think_start < think_end and think_start_num == 1 and think_end_num == 1:
        response_start = think_end + len(eot_token)
        success = True
...
def punish_format_return_positions(text, config):
    ...
    if is_vlm:
        if config.data.get('task_type') == 'VLM_GUI':
            ## format reward should be designed by chat template
            raise NotImplementedError
        else:
            think_template = os.getenv("THINK_TEMPLATE", "v3")
            print("[debug think_template 2 ]", think_template)
            if think_template == 'v1':
            ...
            elif think_template == 'v2' or think_template == 'v3':  ## v2 and v3 use same template with different sp tokens
                think_start = text.find(get_special_tokens_dict_or_name("think_start_token"), 0)
                think_end = text.find(get_special_tokens_dict_or_name("think_end_token"), 0)
...
def match_visual_cot_format(response: str, verifier_feature: dict, allow_last_turn_fc: bool = False) -> bool:
    ...
    def check_think_and_fc_format(s: str) -> tuple[bool, str]:
        ...
        if count_fc_start_token == 1:
            ...
            for pos in [pos_fc_start_token, pos_fc_end_token]:
                if pos_think_start_token <= pos < pos_think_end_token + len(think_end_token):
                    return False, tool_name

            fc_cmd = s[pos_fc_start_token + len(fc_start_token):pos_fc_end_token]
            try:
                signature_list = json.loads(fc_cmd)
    ...
    def add_dummy_think(s: str) -> str:
        """
        如果没有think start token和think end token，则添加一个dummy的think start token和 think end token，仅当no_thinking_required=True时使用
        """
    ...
    visual_tools = ("POINT", "GROUNDING", "ROTATE", "ZOOM")
