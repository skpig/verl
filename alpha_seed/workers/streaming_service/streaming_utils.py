...
def record_xperf_metrics(batch_info, metrics, logger, global_step, prefix=''):
    ...
    dec_bs = [bs for bs in xperf_metrics.get('dec_bs', []) if bs > 0]
    metrics[f'rollout/{prefix}/decode_bs_avg'] = 0 if len(dec_bs) == 0 else (sum(dec_bs) / len(dec_bs))
    ...
    metrics[f'rollout/{prefix}/roofline'] = xperf_metrics.get('roofline', 0)
...
def metrics_for_recommend_standalone_usage(metrics):
    ...
    for i in range(100):
        ...
        if bs_idx is not None:
            metrics['max_kv_util_for_complete_ratio'][complete_ratio] = max(metrics['kv_cache_utils'][bs_idx:])
            metrics['hybrid_latency_with_complete_ratio'][complete_ratio] = cumulative_latency[bs_idx] / 1000
    return metrics
