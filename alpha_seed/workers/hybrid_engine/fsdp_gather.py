...
def ulysses_pad(input_ids_rmpad: torch.Tensor, position_ids_rmpad: Optional[torch.Tensor], sp_size: int):
...
def ulysses_pad_router_logits(input_ids_rmpad: torch.Tensor, position_ids_rmpad: Optional[torch.Tensor], sp_size: int):
    if position_ids_rmpad is not None:
        assert position_ids_rmpad.size(0) == 1
        assert input_ids_rmpad.size(1) == position_ids_rmpad.size(1)
    if sp_size <= 1:
        return input_ids_rmpad, position_ids_rmpad, 0
    _, total_s, _, n_expert = input_ids_rmpad.shape
    pad_size = (sp_size - total_s % sp_size) % sp_size
    if pad_size > 0:
        pad_val = 1. / n_expert
        input_ids_rmpad = torch.nn.functional.pad(input_ids_rmpad, (0, pad_size), value=0)
        input_ids_rmpad = torch.nn.functional.pad(input_ids_rmpad, (0, 0, 0, 0, 0, pad_size), value=pad_val)
        if position_ids_rmpad is not None:
            pad_pos_ids = torch.arange(pad_size, device=position_ids_rmpad.device).unsqueeze(0)
            position_ids_rmpad = torch.cat((position_ids_rmpad, pad_pos_ids), dim=-1)
    return input_ids_rmpad, position_ids_rmpad, pad_size
...
def ulysses_pad_and_slice_inputs(input_ids_rmpad: torch.Tensor,
    ...
    Returns:
        ...
        int: pad size 
    """
    _pad_func = ulysses_pad if not is_router_logits else ulysses_pad_router_logits
    input_ids_rmpad, position_ids_rmpad, pad_size = _pad_func(input_ids_rmpad, position_ids_rmpad, sp_size)
