...
def apply_monkey_patch_to_m8():
    from seed_models.models.m8.modeling_m8 import M8FlashAttention2, M8FusedMoeBlock, M8PreTrainedModel, M8DecoderLayer, M8Model, M8ForCausalLM
    from .modeling_m8 import flash_attn2_rmpad_forward, _fused_moe_ep_forward_without_gate, _fused_moe_ep_forward, release_m8_kv_mirror, decoder_layer_forward, m8_casual_lm_forward
    M8FlashAttention2.forward = flash_attn2_rmpad_forward
    M8FusedMoeBlock.forward = _fused_moe_ep_forward_without_gate
    M8DecoderLayer.forward = decoder_layer_forward
