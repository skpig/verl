...
class DataParallelOnlineRFTActor(BasePPOActor):
    ...
    def __init__(
        ...
        self.ulysses_sequence_parallel_size = self.config.ulysses_sequence_parallel_size
        self.use_ulysses_sp = self.ulysses_sequence_parallel_size > 1
        assert not self.use_ulysses_sp, "Online RFT does not support Ulysses SP"
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    attention_mask = data['attention_mask']
                    response_mask = attention_mask[:, -response_length:]
                    seq_level_reward = data['seq_level_scores'] # (bsz, )
                    ...
                    if self.config.use_dynamic_bsz:
                        # relative to the dynamic bsz
                        loss = nll_loss * (len(data) / self.config.ppo_mini_batch_size)
                    else:
                        loss = nll_loss / self.gradient_accumulation
                    loss.backward()
