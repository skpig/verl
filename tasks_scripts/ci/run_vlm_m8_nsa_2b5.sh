...
export PYTHONPATH=/opt/tiger/native_sparse_attention:$PYTHONPATH

# ckpt和路径
SFT_MODEL_PATH=hdfs://haruna/home/byte_data_seed/lf_lq/user/tianqilin.99/models/m8_nsa/hf
...
default_hdfs_dir=hdfs://haruna/home/byte_data_seed/lf_lq/user/tianqilin.99/test/vlm_grpo
...
# 工程参数
gen_micro_batch_size=16 # use_dynamic_bsz=True时仍然生效
...
python3 tasks/main_ppo.py \
    ...
    +actor_rollout_ref.rollout.num_slots=32 \
    +actor_rollout_ref.rollout.slot_block_size=1024 \
    actor_rollout_ref.rollout.xperf_triton.enable=True \
    actor_rollout_ref.rollout.xperf_triton.max_batch_size=32 \
    +actor_rollout_ref.rollout.xperf_triton.model_name='M8_nsa' \
