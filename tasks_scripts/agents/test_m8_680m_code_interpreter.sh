...
TRAIN_FILE=hdfs://haruna/home/byte_data_seed/lf_lq/user/jiangchengquan/data/math/train_with_ref_ans_code_interpreter.parquet
TEST_FILE=hdfs://haruna/home/byte_data_seed/lf_lq/user/zhangchi.usc1992/data/rlhf/math/test_with_ref_ans.parquet
default_hdfs_dir=hdfs://haruna/home/byte_data_seed/ssd_wlcb/user/jiangchengquan/alphaseed/${EXPERIMENT_NAME}
...
experiment_name='m8_680m_code_interpreter_test'
# 工程参数
...
export EXTERNAL_PLUGIN_PATH=/opt/tiger/agents_extension/plugins
export EXTERNAL_ENV_PATH=/opt/tiger/agents_extension/envs
export EXTERNAL_REWARD_FN_PATH=/opt/tiger/agents_extension/reward_fn
if [ ! -d "${EXTERNAL_PLUGIN_PATH}" ]; then
    echo "${EXTERNAL_PLUGIN_PATH} dose not exist!"
    exit 1
fi
if [ ! -d "${EXTERNAL_ENV_PATH}" ]; then
    echo "${EXTERNAL_ENV_PATH} dose not exist!"
    exit 1
fi
if [ ! -d "${EXTERNAL_REWARD_FN_PATH}" ]; then
    echo "${EXTERNAL_REWARD_FN_PATH} dose not exist!"
    exit 1
fi
...
python3 tasks/main_ppo.py \
    ...
    actor_rollout_ref.rollout.plugin.enable=True \
    actor_rollout_ref.rollout.plugin.names=['code_interpreter'] \
    actor_rollout_ref.rollout.plugin.args='''{'code_interpreter':{'timeout':10}}''' \
    actor_rollout_ref.rollout.plugin.pause_condition=on_trigger \
    ++actor_rollout_ref.rollout.val_generate_kwargs.plugin_config.enable=False \
