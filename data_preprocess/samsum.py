...
def make_prefix(dialogue, template_type):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--local_dir', default='/data/datasets/samsum')
    ...
    raw_dataset = load_dataset('knkarthick/samsum', split='test')

    test_dataset = raw_dataset.select(range(TEST_SIZE))

    def make_map_fn(split):
        def process_fn(example, idx):
            question = make_prefix(example['dialogue'], template_type=args.template_type)
            solution = example['summary']
            data = {
                "data_source": "samsum",
                "prompt": question,
